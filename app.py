...
uploaded_files = st.file_uploader(
    "Upload one or two MT Quant exports",
    type=["csv", "xlsx", "xlsm"],
    accept_multiple_files=True,
    help="Upload up to two original report-style exports or workbooks generated by this app.",
)

if not uploaded_files:
    st.markdown('<div class="hint">Drop one file for analysis, or two files for a side-by-side comparison. The app automatically includes portfolios such as <b>SL_REX1</b>, <b>TGT_REX2</b>, and later re-entries.</div>', unsafe_allow_html=True)
    st.stop()
if len(uploaded_files) > 2:
    st.error("Please upload a maximum of two files at a time.")
    st.stop()
